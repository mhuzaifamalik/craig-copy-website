  ...
  return (
    <>
      ...
      <div className="info">
        <h4>{data.name}</h4>
        <h5>
          ${data.saleprice ? data.saleprice.toFixed(2) : data.price.toFixed(2)}{" "}
          x {data.quantity}
        </h5>
      </div>
    </>
...
const OrderSummary = ({ sweetAlert, setOrderData, orderData }) => {
  ...
  useEffect(() => {
    if (orderData.state) {
      debugger;
      const taxRate =
        salesTax.find((item) => item.State === orderData.state)?.combinedRate ||
        0;
    ...
    <div className="order-summary">
      ...
      <ul className="cart-products">
        {loading ? (
          ...
          <>
            {cartProductObj.map((item) => (
              <li>
                <CartItemCart data={item} key={item.id} />
              </li>
            ))}
          </>
        ) : (
