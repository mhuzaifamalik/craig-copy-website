    ...
    const PaymentForm = ({ setActiveStep, setCompletedSteps, checkedValue, setCheckedValue, orderData, sweetAlert, setOrderData }) => {
        ...
        useEffect(() => {
                        ...
                        try {
                            ...
                            if (result.status === 'OK') {
                                ...
                                const response = await fetch('/api/order/new', {
                                    ...
                                    body: JSON.stringify({ ...orderData, products: cartProducts, paymentType: checkedValue, token })
                                });
                            ...
                            sweetAlert('error', error.message || 'An error occurred during payment. Please try again.');
                        } finally {
            ...
            return () => {
                ...
                if (card) {
                    card.destroy().catch((e) => console.error('Error cleaning up card:', e));
                }
        ...
        const makePayment = async () => {
            const response = await fetch('/api/order/new', {
                ...
                body: JSON.stringify({ ...orderData, products: cartProducts, paymentType: checkedValue })
            });
                    ...
                    <ul>
                        <li>
                            <div className="radio-wrapper">
                                <input type="radio" name="payment-type" id="del-0" checked
                                    onChange={(e) => {
                                        if (e.target.checked) {
                                ...
                                />
                                <label htmlFor="del-0"><FaCreditCard /> Credit Card</label>
                                <br />
                                <div id="card-container"></div>
                            </div>
                            {checkedValue === 'Credit Card' && <div className="content">
                                {/* <StripeForm /> */}
                                <button id='card-button'
                                    // onClick={makePayment}
                                    className='submit-btn'>Proceed</button>
                            </div>}
                        </li>
                        {/* <li>
                            <div className="radio-wrapper">
